...
3. Build: `npm install`
4. Start: `npm start`
5. Env: `MONGO_URI` (Atlas), `JWT_SECRET`
6. Get URL: `https://your-app.onrender.com`

### Frontend (Vercel/Netlify)
1. Deploy static frontend folder
2. Set env: `VITE_API_URL=https://your-backend.onrender.com/api` (if using Vite)
3. Update API_BASE to backend URL

### MongoDB Atlas (Free)
1. cloud.mongodb.com → New Cluster (M0 free)
2. Create DB user
3. Network → Allow all IPs
4. Connection string: `mongodb+srv://user:pass@cluster.mongodb.net/ecommerce`

### Full Stack Vercel (API Routes)
Create `vercel.json` root:
```json
{
  "rewrites": [{ "source": "/api/(.*)", "destination": "/backend/server.js" }]
}
```

## 🧪 Verify Deployment
```
Backend: /api/products → 9 products
Frontend: Products page loads
Cart/Add/Checkout work
Images display
```

---
*Generated by BLACKBOXAI - Deploy Ready*
